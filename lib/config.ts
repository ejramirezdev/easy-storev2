...
export const PAGINATION_CONFIG = {
  ...
  featuredProductsLimit: parseInt(
    process.env.NEXT_PUBLIC_FEATURED_PRODUCTS_LIMIT || "6",
    10
  ),

  // Productos relacionados
  relatedProductsLimit: parseInt(
    process.env.NEXT_PUBLIC_RELATED_PRODUCTS_LIMIT || "10",
    10
  ),
...
export const CONNECTION_CONFIG = {
  ...
  payphoneConfirmationTimeout: parseInt(
    process.env.PAYPHONE_CONFIRMATION_TIMEOUT || "5",
    10
  ),
} as const;
...
export const ADMIN_CONFIG = {
  ...
  defaultAdminEmails: (
    process.env.ADMIN_DEFAULT_EMAILS || "ejramirezdev@gmail.com"
  )
    .split(",")
    .map((e) => e.trim()),
} as const;
...
export const SEO_CONFIG = {
  defaultTitle:
    ...
    "Easy Store - Productos Tecnológicos en Ecuador",
  defaultDescription:
...
export function validateConfig() {
  ...
  if (process.env.NODE_ENV === "production") {
    ...
    if (!process.env.NEXT_PUBLIC_SITE_URL) {
      console.warn(
        "⚠️  NEXT_PUBLIC_SITE_URL no está configurada, usando valor por defecto"
      );
    }
  }
